...
class SlashVerify(commands.Cog):
    ...
    async def info(self, interaction: discord.Interaction, user: discord.Member):
        if not any(role.id in [ug.load_config_value("admin"), ug.load_config_value("mod")] for role in interaction.user.roles):
        ...
        if not verRes:
            return await interaction.response.send_message("This user is not verified yet", ephemeral=False)

        batchRes = batch_collection.find_one({"PRN": verRes["PRN"]})

        if not batchRes:
            return await interaction.response.send_message("Missing data!!!", ephemeral=False)
    ...
    async def info_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.CommandInvokeError):
            if isinstance(error.original, discord.NotFound):
                await interaction.response.send_message("The specified user does not exist or is not in the server", ephemeral=False)
            else:
                await interaction.response.send_message(embed=ug.build_unknown_error_embed(error))
        else:
            await interaction.response.send_message(embed=ug.build_unknown_error_embed(error))
    ...
    async def deverify(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if result.deleted_count == 0:
            return await interaction.response.send_message("This user was not verified in the first place", ephemeral=False)
        ...
        try:
            ...
            return await interaction.response.send_message("I am unable to remove roles from this user although they were deverified. Please check my permissions", ephemeral=False)
